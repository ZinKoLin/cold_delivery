...
@Composable fun HomeDashboard(onNewDelivery: () -> Unit, onPrice: () -> Unit = {}, vm: HomeViewModel = hiltViewModel()) {
    ...
    LazyColumn(Modifier.fillMaxSize().padding(horizontal = 20.dp), verticalArrangement = Arrangement.spacedBy(14.dp), contentPadding = PaddingValues(vertical = 18.dp)) {
        item { Text("${vm.today.dayOfWeek.name.lowercase().replaceFirstChar { it.uppercase() }} • ${vm.today.format(DateTimeFormatter.ofPattern("dd MMM yyyy"))}", style = MaterialTheme.typography.titleMedium, color = DeepRed); Text("Cold Delivery", style = MaterialTheme.typography.headlineLarge) }
        item { Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) { SummaryStatCard("Customers", state.todayCustomers.size.toString(), Modifier.weight(1f)); SummaryStatCard("Delivered", delivered.toString(), Modifier.weight(1f)); SummaryStatCard("Pending", pending.toString(), Modifier.weight(1f)) } }
        item { ColdDeliveryCard(Modifier.fillMaxWidth()) { Column(Modifier.padding(18.dp)) { Text("Today's Price", style = MaterialTheme.typography.titleMedium); Text("Set one shared price per product for today", color = Color.DarkGray); Spacer(Modifier.height(8.dp)); PrimaryRedButton("Open Today's Price", onPrice) } } }
        item { Text(if (state.todayCustomers.isEmpty()) "Closed Day" else "Today's Customers", style = MaterialTheme.typography.titleLarge) }
...
@Composable fun CustomersScreen(onAdd: () -> Unit = {}, onOpen: (Long) -> Unit = {}, vm: HomeViewModel = hiltViewModel()) { val state by vm.state.collectAsState(); var query by remember { mutableStateOf("") }; var selectedDay by remember { mutableStateOf<DeliveryDay?>(null) }; val customers = state.customers.filter { it.name.contains(query, true) && (selectedDay == null || it.deliveryDay == selectedDay) }; PremiumPage { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) { PremiumPageTitle("Customers", "Manage customers and delivery days"); TextButton(onClick = onAdd) { Text("+ Add customer", color = DeepRed, fontSize = 12.sp) } }; Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) { DeliveryDay.values().forEach { d -> FilterChip(selectedDay == d, { selectedDay = if (selectedDay == d) null else d }, label = { Text(d.name.take(3).lowercase().replaceFirstChar { it.uppercase() }, fontSize = 10.sp) }) } }; OutlinedTextField(query, { query = it }, Modifier.fillMaxWidth().heightIn(min = 50.dp), label = { Text("Search customer") }, singleLine = true, shape = com.colddelivery.app.ui.theme.ColdDeliveryShapes.Input); Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) { SummaryStatCard("Customers", customers.size.toString(), Modifier.weight(1f)); SummaryStatCard("Today", state.todayCustomers.size.toString(), Modifier.weight(1f)) }; LazyColumn(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(8.dp), contentPadding = PaddingValues(bottom = 12.dp)) { items(customers, key = { it.id }) { customer -> ColdDeliveryCard(Modifier.fillMaxWidth().clickable { onOpen(customer.id) }) { Row(Modifier.fillMaxWidth().padding(11.dp), verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) { Surface(Modifier.size(36.dp), shape = androidx.compose.foundation.shape.CircleShape, color = DeepRed.copy(alpha = .09f)) { Box(contentAlignment = androidx.compose.ui.Alignment.Center) { Text("⌂", color = DeepRed, fontSize = 21.sp) } }; Spacer(Modifier.width(9.dp)); Column(Modifier.weight(1f)) { Text(customer.name, fontSize = 14.sp, fontWeight = androidx.compose.ui.text.font.FontWeight.Bold, maxLines = 1); Text(customer.phone, color = ColdDeliveryColors.SecondaryText, fontSize = 10.sp); Text(customer.address, color = ColdDeliveryColors.SecondaryText, fontSize = 9.sp, maxLines = 1, overflow = androidx.compose.ui.text.style.TextOverflow.Ellipsis) }; Column(horizontalAlignment = androidx.compose.ui.Alignment.End) { Text(customer.deliveryDay.name.take(3), color = Gold, fontSize = 9.sp); StatusChip(state.deliveredCustomerIds.contains(customer.id)) } } } } } } }

@Composable fun StockScreen(onAddProduct: () -> Unit = {}, onStockIn: () -> Unit = {}, onOpen: (Long) -> Unit = {}, vm: FeatureViewModel = hiltViewModel()) { val products by vm.products.collectAsState(); val threshold by vm.lowStockThreshold.collectAsState(); var summaries by remember { mutableStateOf<Map<Long, List<StockBatchEntity>>>(emptyMap()) }; LaunchedEffect(products) { summaries = products.associate { it.id to vm.stockBatches(it.id) } }; val total = summaries.values.flatten().sumOf { it.remainingQty }; PremiumPage { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) { PremiumPageTitle("Stock", "FIFO inventory at a glance"); Row { TextButton(onClick = onStockIn) { Text("Stock In", color = DeepRed, fontSize = 11.sp) }; TextButton(onClick = onAddProduct) { Text("+ Product", color = DeepRed, fontSize = 11.sp) } } }; Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) { SummaryStatCard("Products", products.size.toString(), Modifier.weight(1f).height(84.dp)); SummaryStatCard("Cartons", total.toString(), Modifier.weight(1f).height(84.dp)); SummaryStatCard("Alerts", summaries.count { it.value.sumOf(StockBatchEntity::remainingQty) <= threshold }.toString(), Modifier.weight(1f).height(84.dp)) }; ColdDeliveryCard(Modifier.fillMaxWidth().heightIn(min = 54.dp)) { Row(Modifier.padding(12.dp), verticalAlignment = androidx.compose.ui.Alignment.CenterVertically) { Text("⚠", color = DeepRed, fontSize = 20.sp); Spacer(Modifier.width(8.dp)); Text("Stock Alert", color = DeepRed, fontWeight = androidx.compose.ui.text.font.FontWeight.Bold); Spacer(Modifier.weight(1f)); Text("${summaries.count { it.value.sumOf(StockBatchEntity::remainingQty) <= threshold }} products need attention", color = ColdDeliveryColors.SecondaryText, fontSize = 10.sp) } }; LazyColumn(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(8.dp), contentPadding = PaddingValues(bottom = 12.dp)) { items(products, key = { it.id }) { product -> val batches = summaries[product.id].orEmpty(); val remaining = batches.sumOf { it.remainingQty }; val oldest = batches.minOfOrNull { it.stockInDate }; ColdDeliveryCard(Modifier.fillMaxWidth().clickable { onOpen(product.id) }) { Column(Modifier.padding(12.dp)) { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Text(product.productCode, color = Gold, fontSize = 11.sp); Text(if (remaining == 0) "ZERO STOCK" else if (remaining <= threshold) "LOW STOCK" else "AVAILABLE", color = if (remaining <= threshold) DeepRed else DeliveredGreen, fontSize = 10.sp, fontWeight = androidx.compose.ui.text.font.FontWeight.Bold) }; Text(product.productName, fontSize = 15.sp, fontWeight = androidx.compose.ui.text.font.FontWeight.Bold); Text("$remaining CTN  •  ${batches.size} batches  •  oldest ${oldest?.let(::formatDay) ?: "—"}", color = if (remaining == 0) DeepRed else ColdDeliveryColors.SecondaryText, fontSize = 10.sp) } } } } } }
                    ...
                    ColdDeliveryCard(Modifier.fillMaxWidth().clickable { onOpen(delivery.id) }) {
                        Row(Modifier.fillMaxWidth().padding(13.dp), verticalAlignment = Alignment.CenterVertically) {
                            ...
                            Column(Modifier.weight(1f)) {
                                Text(customer?.name ?: "Delivery", fontSize = 15.sp, fontWeight = FontWeight.Bold, maxLines = 1, overflow = TextOverflow.Ellipsis)
                                ...
                                Row(Modifier.padding(top = 5.dp), horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                                    itemProducts[delivery.id].orEmpty().take(3).forEach { productIdForTag ->
                                        ...
                                        Surface(shape = RoundedCornerShape(50), color = Gold.copy(alpha = .13f)) { Text(product?.productName ?: "Product", Modifier.padding(horizontal = 6.dp, vertical = 2.dp), color = ColdDeliveryColors.SecondaryText, fontSize = 8.sp, maxLines = 1, overflow = TextOverflow.Ellipsis) }
                                    }
                                }
                            }
